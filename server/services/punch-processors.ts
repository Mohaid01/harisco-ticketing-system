...
export async function processAttendancePunch(input: {
  ...
  const { userId, punchTime, scanMethod, attendStat = 'None', deviceLabel = 'Device' } = input;

  if (userId === '0' || userId === '00000000') {
    logger.security(`⚠️ [SECURITY] Dropped a failed/unregistered scan attempt from ${deviceLabel}.`);
    return;
  }
  ...
  try {
    ...
    const userDoc = await db.get<{ name: string }>('SELECT name FROM users WHERE LOWER(username) = ? OR id = ?', [
    ...
    logger.error(`⚠️ [DB USER LOOKUP/STATUS ERROR] Falling back to default values from ${deviceLabel}:`, lookupError);
  ...
  try {
    ...
    if (holiday) {
      logger.info(`🏖️ [HOLIDAY] Scan on '${holiday.name}' (${scanDateStr}) — ignored from ${deviceLabel}.`);
      return;
    }
...
export async function processFactoryAttendancePunch(input: {
  ...
  if (userId === '0' || userId === '00000000') {
    logger.security(`⚠️ [SECURITY] Dropped a failed/unregistered factory scan attempt from ${deviceLabel}.`);
    return;
  }
  ...
  try {
    ...
    logger.info(`[DB] Saved factory log profile successfully from ${deviceLabel}.`);

    // Broadcast the new log to all connected SSE clients
    try {
      const newLog = await db.get('SELECT * FROM factory_attendance_logs WHERE id = ?', insertResult.lastID);
      if (newLog) {
        const message = `data: ${JSON.stringify(newLog)}\n\n`;
        sseClients.broadcast(message);
      }
    ...
    logger.error(`❌ [DB] Factory Database Storage Error from ${deviceLabel}:`, err);
  }
}
