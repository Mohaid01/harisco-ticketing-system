...
router.get(
  "/",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<NoticesResponse>) => {
    try {
      const db = getDb();
...
router.post(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<CreateNoticeRequestBody>,
    res: ApiResponse<CreateNoticeResponse>,
  ) => {
    if (
      req.user?.role !== "it" &&
...
router.put(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<UpdateNoticeRequestBody>,
    res: ApiResponse<UpdateNoticeResponse>,
  ) => {
    if (
      req.user?.role !== "it" &&
      req.user?.role !== "manager" &&
...
router.delete(
  "/:id",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<DeleteNoticeResponse>) => {
    if (
      req.user?.role !== "it" &&
      req.user?.role !== "manager" &&
      req.user?.role !== "executive"
