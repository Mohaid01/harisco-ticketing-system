...
import { loginLimiter, JWT_SECRET } from "../constants.ts";
...
router.post(
  ...
  loginLimiter,
  async (
    req: ApiRequest<LoginRequestBody>,
...
router.get(
  "/me",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<MeResponse>) => {
    try {
      const db = getDb();
      let user = await db.get<DbUser>(
...
router.post(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<ResetPasswordRequestBody>,
...
router.post(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<ChangePasswordRequestBody>,
