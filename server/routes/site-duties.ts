...
// GET /api/site-duties
router.get('/', authenticateToken, async (req: AuthRequest, res: ApiResponse<SiteDutiesResponse>) => {
...
router.post(
  '/',
  authenticateToken,
  async (req: ApiAuthRequest<CreateSiteDutyRequestBody>, res: ApiResponse<CreateSiteDutyResponse>) => {
    ...
    try {
      ...
      if (existingDuty) {
        res.status(400).json({
          error: 'Cannot apply for site duty on a date with an already approved site duty.',
        });
        return;
      }
      ...
      await db.run(
        ...
        [id, req.user?.id, req.user?.name, siteName, reason, startDate, endDate, 'pending', timestamp]
      );
...
router.put(
  '/:id/status',
  authenticateToken,
  async (req: ApiAuthRequest<UpdateSiteDutyStatusRequestBody>, res: ApiResponse<UpdateSiteDutyStatusResponse>) => {
    const db = getDb();
    ...
    try {
      ...
      await db.run('UPDATE site_duty_applications SET status = ? WHERE id = ?', [status, sdId]);
