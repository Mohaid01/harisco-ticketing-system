...
// GET /api/holidays
router.get('/', authenticateToken, async (req: AuthRequest, res: ApiResponse<HolidaysResponse>) => {
  try {
    const db = getDb();
...
router.post(
  '/',
  authenticateToken,
  async (req: ApiAuthRequest<CreateHolidayRequestBody>, res: ApiResponse<CreateHolidayResponse>) => {
...
router.delete('/:date', authenticateToken, async (req: AuthRequest, res: ApiResponse<DeleteHolidayResponse>) => {
  if (req.user?.role !== 'it' && req.user?.role !== 'manager') {
    res.status(403).json({ error: 'Forbidden.' });
    return;
  }
