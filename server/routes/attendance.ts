...
router.get(
  "/",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<AttendanceLogsResponse>) => {
    try {
      const db = getDb();
...
router.post(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<AddManualAttendanceRequestBody>,
    res: ApiResponse<AddManualAttendanceResponse>,
  ) => {
...
router.delete(
  "/",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<ClearAttendanceResponse>) => {
    if (req.user?.role !== "it") {
      res.status(403).json({
...
router.delete(
  "/:id",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<DeleteAttendanceLogResponse>) => {
