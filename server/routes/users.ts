...
router.get(
  "/",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<UsersResponse>) => {
    try {
      const db = getDb();
...
router.post(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<CreateUserRequestBody>,
...
router.delete(
  "/:id",
  authenticateToken,
  async (req: AuthRequest, res: ApiResponse<DeleteUserResponse>) => {
    if (req.user?.role !== "it") {
      res.status(403).json({
...
router.post(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<ResetUserPasswordRequestBody>,
    res: ApiResponse<ResetUserPasswordResponse>,
  ) => {
    if (req.user?.role !== "it") {
      res.status(403).json({
...
router.put(
  ...
  authenticateToken,
  async (
    req: ApiAuthRequest<UpdateUserRequestBody>,
    res: ApiResponse<UpdateUserResponse>,
  ) => {
    if (req.user?.role !== "it") {
      res.status(403).json({
