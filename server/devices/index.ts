...
export function setupDeviceHandlers(app: express.Express, server: any) {
  ...
  wss.on("connection", (ws: any, req: any) => {
    logger.info(
      `ðŸ“¡ [DEVICE CONNECTED] Connection open from IP: ${req.socket.remoteAddress}`,
    );

    logger.info("Device connected", {
      ip: req.socket.remoteAddress,
      firmware:
        ws.upgrade?.request?.headers?.["x-device-firmware"] ||
        "unknown",
    });
    ...
    ws.on("close", () =>
      logger.info("ðŸ”Œ [DEVICE DISCONNECTED] Channel closed."),
    );
  });
}