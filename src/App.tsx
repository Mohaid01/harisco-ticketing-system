...
function App() {
  ...
  const handleAddUser = async (data: { name: string; email: string; username: string; role: UserRole; password?: string }) => {
    if (!token || !currentUser) return;
    try {
      ...
      <main className="main-content">
        ...
        <header className="app-header" style={{ justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: 'var(--status-closed)' }}></span>
          </div>
        </header>
