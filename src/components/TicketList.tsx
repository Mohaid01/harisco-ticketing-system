...
export const TicketList: React.FC<TicketListProps> = ({
  ...
  const filteredTickets = useMemo(() => {
    return tickets
      .filter((ticket) => {
        ...
        if (currentUser.role === 'employee' && ticket.reporterId !== currentUser.id) {
          return false;
        }
  ...
  const getStatusBadge = (status: TicketStatus) => {
    switch (status) {
      case 'open':
        return <span className="badge badge-progress" style={{ backgroundColor: 'rgba(14, 82, 155, 0.12)', color: '#0e529b' }}>Open</span>;
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <select
              ...
              className="form-input"
              style={{ width: '150px', height: '38px', padding: '6px 12px', backgroundColor: 'var(--bg-primary)' }}
              value={typeFilter}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <select
              ...
              className="form-input"
              style={{ width: '180px', height: '38px', padding: '6px 12px', backgroundColor: 'var(--bg-primary)' }}
              value={statusFilter}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: 'auto' }}>
            ...
            <select
              ...
              className="form-input"
              style={{ width: '130px', height: '38px', padding: '6px 12px', backgroundColor: 'var(--bg-primary)' }}
              value={sortBy}
            ...
            <tbody>
              {filteredTickets.map((ticket) => (
                <tr key={ticket.id} onClick={() => onSelectTicket(ticket.id)}>
                  ...
                  <td style={{ fontSize: '0.78rem', color: 'var(--text-secondary)' }}>{formatDate(ticket.createdAt)}</td>
                </tr>
              ))}
