...
import { Clock, CheckSquare, Settings, ShieldCheck, UserCheck } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const awaitingIt = roleTickets.filter((t) => t.status === 'awaiting_it_approval').length;
  const awaitingManager = roleTickets.filter((t) => t.status === 'awaiting_manager_approval').length;
  const open = roleTickets.filter((t) => t.status === 'open').length;
  const awaitingHandover = roleTickets.filter((t) => t.status === 'awaiting_handover').length;
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <p className="page-subtitle">
            Welcome back, <strong>{currentUser.name}</strong>. Here is your ticketing queue overview.
          </p>
        </div>
      ...
      <div className="dashboard-two-col">
        ...
        <div className="panel" style={{ minHeight: '340px' }}>
          <div className="panel-header">
            <h2 className="panel-title">
              {currentUser.role === 'employee' ? 'My Active Tickets' : 'Active System Tickets'}
            </h2>
            <button
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div className="panel" style={{ backgroundColor: 'rgba(14, 82, 155, 0.05)' }}>
            <h2 className="panel-title" style={{ color: 'var(--color-primary-solid)', marginBottom: '12px' }}>
              Ticketing Approvals Flow
            </h2>
            <ol style={{ paddingLeft: '16px', fontSize: '0.82rem', display: 'flex', flexDirection: 'column', gap: '8px', color: 'var(--text-secondary)' }}>
              <li>
